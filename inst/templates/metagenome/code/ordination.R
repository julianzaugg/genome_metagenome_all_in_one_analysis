...
for (i in seq_len(nrow(datasets.df))){
  ...
  for (method.s in names(methods.l)){
    ...
    ordination.gg <- gmaio::plot_ordination(ordination, metadata.df, colour_by = colour_by.s,
                                            colours.v = project.l$palettes[[colour_by.s]], label_by = "Sample_label",
                                            ellipse = !is.null(group.s), title = gsub("__", " ", dataset.s),
                                            caption = caption.s)
